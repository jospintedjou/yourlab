                ...
                @forelse(\App\Models\Project::latest()->take(5)->get() as $project)
                    <div class="flex items-center gap-4 p-3 hover:bg-gray-50 rounded transition">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-sm text-gray-600 truncate">{{ Str::limit($project->description ?? 'Aucune description', 60) }}</p>
                        </div>
                        
                        <!-- Status Badge -->
                        <span class="px-3 py-1 text-xs font-semibold rounded-full bg-{{ $project->status->color() }}-100 text-{{ $project->status->color() }}-800 whitespace-nowrap">
